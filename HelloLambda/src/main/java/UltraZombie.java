...
public abstract class UltraZombie extends Zombie {
	...
	public final boolean isRightClear() {
		...
		synchronized (Zombie.class) {
			try {
				...
				return checkDelta("Wall", dx, dy) == null && checkDelta(null, dx, dy) != this;
			} catch (InterruptedException e) {
				throw new ZombifiedZombieException();
			}
		}
	...
	/**
	 * Check if there is a wall or the edge of the world in a certain direction from
	 * the Zombie.
	 * 
	 * @param direction
	 *            The direction to look for a wall (NORTH, SOUTH, EAST, or WEST)
	...
	/**
	 * Check for an object of a particular class at an offset from the zombie or if
	 * that distance is beyond the edge of the world
	 *
	 * @param classname
	 *            The class to check for. If null, look for the edge of the world
	 * @param dx
	 *            The distance(in cells) along the x-axis to look for the
	 *            object/edge
	 * @param dy
	 *            The distance(in cells) along the y-axis to look for the
	 *            object/edge
	 * @return The object at the offset, or a reference to this zombie if the offset
	 *         is off the edge of the world, null if no object of the given class is
	 *         at that distance or the world does not end within that distance.
	 */
